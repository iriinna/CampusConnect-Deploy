...
const SubjectDetails = () => {
  ...
  const loadSubjectDetails = async () => {
    try {
      const data = await gradesApi.getSubjectById(parseInt(id!));
      setSubject(data);
    } catch (error) {
  ...
  const loadGrades = async () => {
    ...
    try {
      ...
      const data = await gradesApi.getSubjectGrades(parseInt(id!));
  ...
  const loadStudents = async () => {
    try {
      ...
      const response = await fetch('http://localhost:5099/api/User/search', {
        headers: {
  ...
  const filteredStudents = students.filter((student) => {
    ...
    return fullName.includes(query) || student.email.toLowerCase().includes(query) || student.studentId?.toLowerCase().includes(query);
  });
                ...
                <form onSubmit={handleAddGrade} className="space-y-4">
                  ...
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <Input
                        type="text"
                        placeholder="Caută după nume, email sau matricol..."
                        value={searchQuery}
                    ...
                    {searchQuery && !selectedStudent && (
                      <div className="mt-2 max-h-60 overflow-y-auto border rounded-lg">
                        {filteredStudents.length === 0 ? (
                          <p className="p-4 text-muted-foreground text-center">Nu s-au găsit studenți</p>
                        ) : (
                      ...
                      <tr key={grade.id} className="border-b last:border-b-0 hover:bg-accent">
                        <td className="py-3 px-4">
                          <div>
                            ...
                            {grade.studentId_Number && (
                              <p className="text-xs text-gray-500">Matricol: {grade.studentId_Number}</p>
                            )}
                          </div>
                        </td>
