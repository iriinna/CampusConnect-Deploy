...
const MySubjects = () => {
  ...
  const [newSubject, setNewSubject] = useState<CreateSubjectRequest>({ name: '', description: '' });
  ...
  const loadSubjects = async () => {
    setLoading(true);
    try {
      const data = await gradesApi.getMySubjects();
      setSubjects(data);
    } catch (error) {
  ...
  const handleCreateSubject = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      await gradesApi.createSubject(newSubject);
  ...
  const handleDeleteSubject = async (id: number) => {
    ...
    try {
      await gradesApi.deleteSubject(id);
      loadSubjects();
    } catch (error) {
                    ...
                    <div className="flex justify-between items-start">
                      <div className="flex-1">
                        ...
                        <Badge className="text-xs border border-gray-300">
                          ...
                          {subject.gradesCount} {subject.gradesCount === 1 ? 'notă' : 'note'}
                        </Badge>
                      </div>
